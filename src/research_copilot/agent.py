...
- 用户明确要求“找/搜索一篇 Mamba 医学图像分割论文并导入/添加到论文库”时，
  使用 find_arxiv_import_candidate；它会只检索一次、跳过已入库候选、选择最匹配的未入库论文，
  仅在找到新候选时才调用 import_arxiv_paper 请求人工确认。不得先调用 search_arxiv 再自行重复搜索或改为 list_papers。
...
def build_agent(services: ServiceContainer, *, enable_hitl: bool = True):
    ...
    if enable_hitl:
        middleware.append(
            HumanInTheLoopMiddleware(
                interrupt_on={
                    "import_local_paper": True,
                    "import_arxiv_paper": True,
                },
